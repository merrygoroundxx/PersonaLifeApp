...
3.  (Optional) Enter a **Base URL** if you are using a proxy or a compatible service.
4.  **Supported Providers**:
    - **DeepSeek**: Use `https://api.deepseek.com` as Base URL.
    - **Gemini (Google)**: Use `https://generativelanguage.googleapis.com` as Base URL.
5.  Save the contract!
...
3.  (可选) 如果使用代理或兼容服务，请输入 **Base URL**。
4.  **支持的服务商**:
    - **DeepSeek**: Base URL 填写 `https://api.deepseek.com`。
    - **Gemini (Google)**: Base URL 填写 `https://generativelanguage.googleapis.com`。
5.  保存契约！
